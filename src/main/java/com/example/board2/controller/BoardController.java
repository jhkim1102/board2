		...
		try {
			...
			if(searchValue == null) {
				...
				if(request.getMethod().equalsIgnoreCase("Get")) {
					...
					//쿼리 파라메터의 값(searchValue)를 디코딩해준다.
			...
			int start = (currentPage -1 )* numPerPage +1; //1 6 11 16.....(5씩 증가)
			...
			model.addAttribute("articleUrl", articleUrl);	//상세피이지로 이동하기 위한 URL
